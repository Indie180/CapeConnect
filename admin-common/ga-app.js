(function () {
  const KEYS = {
    session: "GA_ADMIN_SESSION",
    users: "GA_USERS",
    tickets: "GA_TICKETS",
    wallets: "GA_WALLETS",
    timetables: "GA_TIMETABLES",
    pricesGlobal: "GA_PRICES_GLOBAL",
    pricesRoutes: "GA_PRICES_ROUTES",
    audit: "GA_AUDIT_LOGS",
    backups: "GA_BACKUPS",
    settings: "GA_ADMIN_SETTINGS",
    seedFlag: "GA_SEEDED_V1"
  };

  const OPERATOR = "Golden Arrow";
  const EXPECTED_ADMIN_EMAIL = "ga-admin@capeconnect.demo";
  const DEFAULT_ADMIN = {
  ...
  const PRODUCT_MAP = {
    five_ride: { label: "5 Ride", journeys: 5 },
    weekly: { label: "Weekly", journeys: 10 },
    monthly: { label: "Monthly", journeys: 48 },
    single: { label: "Single Journey", journeys: 1 }
  };
  ...
  async function login(email, password, phrase) {
    ...
    if (String(phrase || "").trim() !== String(cfg.phrase || "GOLDEN-ADMIN")) {
    ...
    try {
      ...
      if (!isAdmin || !userEmail.includes("ga-admin")) {
        return { ok: false, message: "Account is not authorized for Golden Arrow admin." };
      }
